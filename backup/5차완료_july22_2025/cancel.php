...
if (file_exists('payment_errors.log')) {
    ...
    foreach ($lines as $line) {
        // 다양한 성공 패턴을 확인 (3001, 0000 등)
        if (strpos($line, 'Approval Request/Response') !== false && 
            (strpos($line, '"resultCd":"3001"') !== false || 
             strpos($line, '"resultCd":"0000"') !== false ||
        ...
        // NOTI 데이터에서 성공한 결제 정보 수집
        elseif (strpos($line, '[NOTI]') !== false && strpos($line, 'SUCCESS') !== false) {
            // NOTI SUCCESS 로그에서 정보 추출
            if (preg_match('/TID: ([^,]+).*주문번호: ([^,]+).*금액: ([^,]+)/', $line, $matches)) {
                $recentPayments[] = [
                    'tid' => trim($matches[1]),
                    'ordNo' => trim($matches[2]),
                    'amt' => trim($matches[3]),
                    'goodsName' => 'NOTI 성공 결제',
                    'ordNm' => '테스트 고객',
                    'appDtm' => date('Y-m-d H:i:s')
                ];
            }
        }
        // NOTI 수신 데이터에서 결제 정보 수집
        elseif (strpos($line, '[NOTI]') !== false && strpos($line, '"resultCd":"3001"') !== false) {
            if (preg_match('/수신 데이터: (.+)$/', $line, $matches)) {
                $notiData = json_decode($matches[1], true);
                if ($notiData && isset($notiData['tid'])) {
                    $recentPayments[] = [
                        'tid' => $notiData['tid'],
                        'ordNo' => $notiData['ordNo'] ?? '미확인',
                        'amt' => $notiData['amt'] ?? '0',
                        'goodsName' => 'NOTI 결제',
                        'ordNm' => '고객',
                        'appDtm' => $notiData['appDt'] ?? date('Ymd')
                    ];
                }
            }
        }
...
foreach ($recentPayments as $payment) {
    if (!empty($payment['tid'])) {
        $uniquePayments[$payment['tid']] = $payment;
    }
}

// 최신 결제부터 표시
$recentPayments = array_reverse(array_values($uniquePayments));
$recentPayments = array_slice($recentPayments, 0, 10); // 최근 10개만

// 디버깅 정보 추가
$debugInfo = [
    'total_found' => count($recentPayments),
    'log_exists' => file_exists('payment_errors.log'),
    'log_size' => file_exists('payment_errors.log') ? filesize('payment_errors.log') : 0
];
?>
<!DOCTYPE html>
...
<body>
                ...
                <div class="card border-0 shadow">
                    ...
                    <div class="card-body p-4">
                        <?php if ($directCancel): ?>
                            ...
                            <div class="alert alert-warning border-0" role="alert">
                                <h6 class="alert-heading">
                                    <i class="fas fa-exclamation-triangle me-2"></i>결제 취소 요청
                                </h6>
                                <p class="mb-0">아래 결제를 취소하시겠습니까?</p>
                            </div>

                            <div class="card bg-dark border-warning mb-4">
                                <div class="card-body">
                                    <h6 class="card-title text-warning mb-3">
                                        <i class="fas fa-receipt me-2"></i>취소 대상 결제
                                    </h6>
                                    <div class="row">
                                        <div class="col-md-6">
                                            <p><strong>거래번호:</strong> <span class="text-info"><?php echo htmlspecialchars($directTid); ?></span></p>
                                        </div>
                                        <div class="col-md-6">
                                            <p><strong>취소금액:</strong> <span class="text-warning"><?php echo number_format($directAmt); ?>원</span></p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="text-center">
                                <button class="btn btn-danger btn-lg me-3" 
                                        onclick="directCancel('<?php echo htmlspecialchars($directTid); ?>', '<?php echo htmlspecialchars($directAmt); ?>')">
                                    <i class="fas fa-times me-2"></i>결제 취소하기
                                </button>
                        ...
                        <?php elseif (empty($recentPayments)): ?>
                            ...
                            <!-- 디버깅 정보 -->
                            <div class="alert alert-secondary border-0 mt-3" role="alert">
                                <h6 class="alert-heading">디버깅 정보</h6>
                                <ul class="mb-0">
                                    <li>로그 파일 존재: <?php echo $debugInfo['log_exists'] ? '예' : '아니오'; ?></li>
                                    <li>로그 파일 크기: <?php echo number_format($debugInfo['log_size']); ?> bytes</li>
                                    <li>찾은 결제 건수: <?php echo $debugInfo['total_found']; ?></li>
                                </ul>
                            </div>
    ...
    <script>
        ...
        // 직접 취소 요청 처리
        function directCancel(tid, amt) {
            if (confirm('정말로 이 결제를 취소하시겠습니까?\n\n거래번호: ' + tid + '\n취소금액: ' + parseInt(amt).toLocaleString() + '원')) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = 'cancel_result.php';
                ...
                // ordNo는 tid에서 추출 (mimich067m01012507111237460372 -> ORD... 형태로 변환)
                const ordNoInput = document.createElement('input');
                ordNoInput.type = 'hidden';
                ordNoInput.name = 'ordNo';
                ordNoInput.value = 'ORD' + tid.substring(10); // 간단한 추정
