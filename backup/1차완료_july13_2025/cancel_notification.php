...
// FINTREE에서 취소 완료 후 백그라운드로 호출하는 NOTI URL
...
function writeNotificationLog($message) {
    ...
    $logMessage = "[CANCEL-NOTI] [{$timestamp}] {$message}" . PHP_EOL;
...
try {
    ...
    // 로그에 기록
    writeNotificationLog("취소 NOTI 수신 시작");
    ...
    $canAmt = isset($postData['canAmt']) ? $postData['canAmt'] : '';
    ...
    if ($resultCd === '0000') {
        // 취소 성공
        writeNotificationLog("SUCCESS: 취소 성공 알림 - TID: $tid, 주문번호: $ordNo, 취소금액: $canAmt");
        
        // 여기에 실제 비즈니스 로직 추가 가능
        // 예: 데이터베이스 업데이트, 재고 복원, 환불 처리 등
        ...
        writeNotificationLog("FAIL: 취소 실패 알림 - TID: $tid, 코드: $resultCd, 메시지: $resultMsg");
        
        // FINTREE에 실패 응답
        echo 'FAIL';
    }
    
    writeNotificationLog("취소 NOTI 처리 완료");
